type ContainerConstType = {
  id: string;
};

type ContainerType = ContainerConstType & {
  id: string;
  element: HTMLElement;
  rect: DOMRect;
  shapes: {
    [key: string]: SVGOjectType;
  };
  ...
  selected: SVGOjectType | null;
  overingShape: SVGOjectType | null;
...
class Container {
  id: ContainerType["id"];
  element: ContainerType["element"];
  rect: ContainerType["rect"];
  shapes: ContainerType["shapes"] = {};

  mousedownTriggered: ContainerType["mousedownTriggered"] = false;
  selectedShape: ContainerType["selectedShape"];
  selected: ContainerType["selected"];
  action: ContainerType["action"] = "";
  placeholder: ContainerType["placeholder"];
  overingShape: ContainerType["overingShape"];

  uniq: ContainerType["uniq"] = 1;

  constructor({ id }: { id: ContainerType["id"] }) {
    const element = document.getElementById(id);
    if (!element) throw new Error("container not defined!");
  ...
  addShape(event) {
    if (!(this.selectedShape in SHAPES)) return;
    const Cls = SHAPES[this.selectedShape] as typeof SVGOject;
  ...
  overing(event: MouseEvent) {
    if (!(event.target instanceof SVGElement)) return;
    const id = event.target.id;

    if (!(id in this.shapes)) return;
  ...
  select(event: MouseEvent) {
    ...
    if (!this.overingShape) return;
    this.selected = this.overingShape;

    if (!this.selected) return;
    if (!this.selected.svg) return;
